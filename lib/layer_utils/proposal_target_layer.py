...
def _sample_rois(all_rois, all_scores, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
  ...
  if fg_inds.size > 0 and bg_inds.size > 0: #如果前景与背景都存在
    ...
    for idx in cp_fg_inds:
          ...
          for i3 in range(3):
            for i4 in range(3):
              ...
              if i1==0 and i2 ==0 and i3 == 0 and i4 == 0:
                continue
              box_temp = all_rois[idx]
              box_add_temp = [0, (i1-1) * 0.1 * (box_temp[3] - box_temp[1]),
                              (i2-1) * 0.1 * (box_temp[3] - box_temp[1]),
                              (i3-1) * 0.1 * (box_temp[4] - box_temp[2]),
      ...
      for i in range(len(box_group_temp)):
        fg_inds = np.append(fg_inds, len(all_rois)) # 将新的box的序号添加到fg_inds
        labels = np.append(labels, np.array(labels[idx]))
  ...
  labels[int(fg_rois_per_image):] = 0 #将background的标签设置为0
  ...
  return labels, rois, roi_scores, bbox_targets, bbox_inside_weights #返回每一个anchor对应的类别信息，rois设定为0.1~1之间的roi,bbox_targets的信息是在其对应坐标上的，bbox_inside_weights的也是
