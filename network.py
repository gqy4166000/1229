...
class Net(object):
    ...
    def proposal_target_layer(self, rois, roi_scores, name):
        ...
        self._proposal_targets['rois'] = rois #注意这里的shape 是【-1，self._num_classes*4】
    ...
    def build_network(self):
        self.net_feature = self.vgg16(self.image)
        self.rois_output = self.rpn_net(self.net_feature, self.num_anchor)
        self.all_anchors = tf.py_func(all_anchor_conner, [self.image_width, self.image_height, cfg.feat_stride], tf.float32)
        self.labels, self.anchor_obj = self._anchor_labels_process(self.gt_boxes, self.all_anchors, self.anchor_batch,\
                                                  cfg.overlaps_max, cfg.overlaps_min, self.image_width, self.image_height)
        ...
        if self.is_training:
            self.rois_coord, rois_score_process = self.rois_process(self.rois_positive_score, self.rpn_bbx,\
                                                 self.image_width, self.image_height, self.all_anchors) #返回采样后的rois 是坐标形式 [0+4个坐标]
        ...
        pool5 = self._crop_pool_layer(self.net_feature, self.rois_coord, "pool5") #返回roi pooling后的rois
    ...
    def _head_to_tail(self, pool5, is_training, reuse=None):
        with tf.variable_scope('vgg_16'):
            pool5_flat = slim.flatten(pool5, scope='flatten') #扁平化操作，一维任然是batch_size
            fc6 = slim.fully_connected(pool5_flat, 4096, scope='fc6') #全连接层
            if is_training:
                fc6 = slim.dropout(fc6, keep_prob=0.5, is_training=True, 
                                   scope='dropout6') #droput out层 如果是训练 添加dropout层
            fc7 = slim.fully_connected(fc6, 4096, scope='fc7')#4096*4096
            if is_training:
                fc7 = slim.dropout(fc7, keep_prob=0.5, is_training=True, 
                            scope='dropout7')#经过两个drop out 实际上就是在特征图上又进行了两个全连接的操作
        return fc7                              
    ...
    def _region_classification(self, fc7, is_training):
        ...
        #cls_prob = self._softmax_layer(cls_score, "cls_prob") #softmax 处理
        cls_prob = tf.nn.softmax(cls_score, name="cls_prob")
        cls_pred = tf.argmax(cls_score, axis=1, name="cls_pred") #得到类别最大类的角标
        bbox_pred = slim.fully_connected(fc7, self.num_classes * 4, 
                                         ...
                                         trainable=is_training,
                                         weights_regularizer=slim.l2_regularizer(0.0005),
                                         activation_fn=None, scope='bbox_pred') #得到bbox_pred 大小为[1,mum,self._num_classes * 4] 也就是说 每一个class上都有一个bbox 
        self._predictions["cls_score"] = cls_score #[1,num,_num_classes] 并且num是roi的数量
        self._predictions["cls_pred"] = cls_pred  #最大的cls_score
        self._predictions["cls_prob"] = cls_prob  #将cls_score 经过softmax处理
        self._predictions["bbox_pred"] = bbox_pred #[1,num,]
...
#rpn网络 输入feature_map,返回rois的cls和bbox
    ...
    def rois_process(self, rpn_score, rpn_bbox, img_width, img_height, anchors):
        if self.is_training:
            pre_nms_topN = cfg.max_rpn_input_num ## NMS之前允许的TOP-N anchors数目 12000
            post_nms_topN = cfg.max_rpn_nms_num## NMS之后允许的TOP-N anchors数目 2000
        else:
            pre_nms_topN = cfg.test_max_rpn_input_num ## 测试时为6000
            post_nms_topN = cfg.test_max_rpn_nms_num  ## 测试时为300
...
#        acnhor_h = tf.cast(anchors[:,3] - anchors[:,1]+1.0, tf.float32)
    ...
    def clip_boxes(self, boxes, img_width, img_height):
          ...
          return tf.stack([b0, b1, b2, b3], axis=1) #修剪到边缘
          
    def _crop_pool_layer(self, bottom, rois, name): #输入的bottom con_v特征图， rois为rois
        with tf.variable_scope(name) as scope:
            batch_ids = tf.squeeze(tf.slice(rois, [0, 0], [-1, 1], name="batch_id"), [1]) #tf.slice 从tensor中取出部分内容,tf.squeeze去掉张量为1的轴
            # Get the normalized coordinates of bounding boxes
            bottom_shape = tf.shape(bottom)
            height = (tf.to_float(bottom_shape[1]) - 1.) * np.float32(self.feat_stride) #得到图片的height和width
            ...
            bboxes = tf.stop_gradient(tf.concat([y1, x1, y2, x2], axis=1)) #阻止x1,y1,x2,y2的反向传播
            pre_pool_size = cfg.POOLING_SIZE * 2 #7*2
            crops = tf.image.crop_and_resize(bottom, bboxes, tf.to_int32(batch_ids), [pre_pool_size, pre_pool_size], name="crops")#roi操作

        return slim.max_pool2d(crops, [2, 2], padding='SAME') #返回经过roi pool后的rois     
